...
mkdir ${ppl_build_dir}
cd ${ppl_build_dir}
cmd="cmake $options -DCMAKE_EXPORT_COMPILE_COMMANDS=ON .. && cmake --build . -j ${processor_num} --config ${build_type} && cmake --build . --target install -j ${processor_num} --config ${build_type}"
echo "cmd -> $cmd"
eval "$cmd"
